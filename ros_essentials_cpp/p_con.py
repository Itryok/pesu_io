...
def draw_shape(publisher):
    rotate(publisher, 15,45, False)
    move(publisher, 2.0,3.0, True)
    rotate(publisher, 15,90, True)
    move(publisher, 2.0,3.0, True)
    rotate(publisher, 15,135, True)
    move(publisher, 2.0,math.sqrt(18), True)
    
def assignment(publisher):
    desired_pose = Pose()
    desired_pose.x = 2;
    desired_pose.y = 2;
    desired_pose.theta = 2;

    go_to_goal(publisher, 2,2)
...
if __name__ == '__main__':
    try:
        ...
        #draw_shape(velocity_publisher)
        assignment(velocity_publisher)
